...
/*   Updated: 2019/10/18 18:04:32 by zytrams          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
** PointSide: Determine which side of
** a line the point is on. Return value: <0, =0 or >0.
*/
...
t_scaler	scaler_init(float data[5])
{
	...
	* (data[4] - data[3]) / (data[2] - data[0]),
	...
	fabsf(data[4] - data[3]), fabsf(data[2] - data[0]),
	(int)((data[1] - 1 - data[0])
