...
/*   Updated: 2019/10/01 23:32:53 by zytrams          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sprobject	*create_test_sprobj(t_engine *eng)
{
	...
	w = eng->sprites_buffer[2]->texture.width;
	h = eng->sprites_buffer[2]->texture.height;
	res->angle = 0;
	x = 0;
	res->position = (t_point_3d){0, 100, 100, 0};
	res->idle.surface = SDL_CreateRGBSurface(0, w, h, 32, (Uint32)0xff000000,
							(Uint32)0x00ff0000, (Uint32)0x0000ff00, (Uint32)0x000000ff);
	pix = (int *)res->idle.surface->pixels;
	while (x < w)
	{
		y = 0;
		while (y < h)
		{
			...
			red = (eng->sprites_buffer[2]->texture.data)[offsetd1];
			green = (eng->sprites_buffer[2]->texture.data)[offsetd1 + 2];
			blue = (eng->sprites_buffer[2]->texture.data)[offsetd1 + 2];
			alpha = (eng->sprites_buffer[2]->texture.data)[offsetd1 + 3];
...
void		animator_render_sprite_object(t_engine *eng, SDL_Surface *surf, t_player plr, t_sprobject *spr_obj, t_item sect, int *zbuff)
{
	...
	w_partcle.b.x = spr_obj->position.x + spr_obj->idle.surface->w / 16;
	w_partcle.b.y = spr_obj->position.y + spr_obj->idle.surface->h / 16;
	...
	if(x1 >= x2 || x2 < sect.sx1 || x1 > sect.sx2)
		return ;
	engine_render_rescale(spr_obj->idle.surface, surf, spr_obj->position.z, zbuff, (t_fix_point_2d){100, 100}, spr_obj->idle.surface->w / 2, spr_obj->idle.surface->h / 2);
}
