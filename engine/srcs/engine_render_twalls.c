...
/*   Updated: 2019/11/23 16:28:06 by zytrams          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <engine.h>

void	engine_render_ts_object_help(t_wall_help2 *data, t_trns_item item)
{
			...
			t_wall_help2 *data, t_render_stacks *stacks)
{
	...
	stacks->helpstack)).status != 0))
	{
		...
		{
			engine_render_ts_object_help(data, item);
			engine_render_twall(eng, ps.surf, data, stacks);
		}
		else
		{
			sect = eng->world->sectors_array
			+ item.sprite_renderstack->sect_id.sectorno;
			aimed = engine_render_sprites_in_sector(sect,
			ps.surf, ps.plr, item.sprite_renderstack);
			if (aimed)
				eng->aim = aimed;
		}
	}
