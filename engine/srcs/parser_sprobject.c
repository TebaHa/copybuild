...
void			engine_read_sprobjects_from_file(t_engine *eng, t_buff *buff)
{
	...
	if ((buff->sprobjects = (t_sprobject *)ft_memalloc(sizeof(t_sprobject) *
	eng->stats.sprobjects_count)) == NULL)
		util_malloc_error("buff->sprobjects");
	i = 0;
